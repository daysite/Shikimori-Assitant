...
import conexion from './lib/jadibots.js' // conexión principal para acceder a `conns` y `conn`
...
// Restaurar automáticamente los subbots al iniciar
async function AutoReloadJadibots(parentConn = conexion.conn) {
  const basePath = 'sessions/' // Ruta base donde están las carpetas de subbots
  let folders = []

  try {
    folders = await readdir(basePath)
  } catch (err) {
    console.error('✧ No se pudo leer la carpeta de sesiones:', err.message)
    return
  }

  for (const folder of folders) {
    if (/parent/.test(folder)) continue // Saltar la carpeta del bot principal
    const credsFile = join(basePath, folder, 'creds.json')

    if (!existsSync(credsFile)) continue
    if (conexion.conns.has(folder)) continue // Ya está cargado

    try {
      await Jadibot(`${folder}@s.whatsapp.net`, parentConn)
      console.log(`✅ Subbot restaurado: ${folder}`)
    } catch (err) {
      console.error(`❌ No se pudo restaurar el subbot ${folder}: ${err.message}`)
    }
  }

  console.log(`✧ Restauración completa. Total subbots activos: ${conexion.conns.size}`)
}
...
async function connectionUpdate(update) {
  ...
  if (connection == 'connecting') {
    ...
    await AutoReloadJadibots(global.conn)
  }
